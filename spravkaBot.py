...
def bot_message(message):
    if message.chat.type == 'private':
        if message.text == 'Лекарства':
            ...
            butt5 = types.KeyboardButton('Углеводы. Потребность')
            butt6 = types.KeyboardButton('Микроэлементы. Потребность')
            butt7 = types.KeyboardButton('Дополнительно')
            butback = types.KeyboardButton('В начало')
            markup.add(butt1, butt2, butt3, butt4, butt5, butt6, butt7, butback)
            ...
            bot.send_photo(message.chat.id, photo)
        elif message.text == 'Углеводы. Потребность':
            markup = types.ReplyKeyboardMarkup(resize_keyboard=True)
            bot.send_message(message.chat.id, 'Базовая продукция 3.0-5.5мг/кг*мин\n'
                                              'Первые сутки: min 4мг/кг*мин. max до 8мг/кг*мин\n'
                                              'Увеличение на 0.5-1мг/кг*мин, не более 12мг/кг*мин. Глюкоза <3ммоль/л +1мг/кг*мин.\n'
                                              'Уровень глюкозы поддерживать 2.8-4.4\n'
                                              'Гипергликемия выше 10ммоль/л в ОРИТ связана с повышенной заболеваемостью и смертностью.\n'
                                              'Усвоение глюкозы значительно улучшается при корректном введении аминокислот.\n'
                                              'В периферический катетер не более 12.5%')
        elif message.text == 'Микроэлементы. Потребность':
            markup = types.ReplyKeyboardMarkup(resize_keyboard=True)
            bot.send_message(message.chat.id, 'Назначение К при конц.<4.5ммоль/л или норм.диурезе (3-4сут).'
                                              'К началу 2й нед. - 3-4ммоль/кг*сут.'
                                              'Гиперкалиемия - более 6.5ммоль/л, а после 7д - 5.5ммоль/л.\n'
                                              '\n'
                                              'Na - планово с 3-4сут или раньше, если <140ммоль/л.'
                                              'Потребность - 3-5ммоль/кг*сут'
                                              'Уровень Na в норме 125-145ммоль/л.\n'
                                              '\n'
                                              'Mg - в норме 0.7-1.1ммоль/л, но истинный дефицит выявить сложно: в сыворотке только 0.3%.'
                                              'Начало со 2-3 сут - 0.2-0.3ммоль/кг*сут'
                                              'Симптоматическая гипоMg-емия: MgSo4 25% - 0.1-0.2мл/кг за 2-4ч (до 3 р/сут). Разведение 1:5\n'
                                              'Фосфор Цинк и Селен - в/в препаратов, зарегистрированных в России нет.\n'
                                              '\n'
                                              'Витамины. Виталипид Н (жирораст.А, D2, Е, К1) - 4мл/кг.\n'
                                              'Солувит Н (водораст.В1, В2, В3, В5, В6, В7, В9, В12, С) - 1мл/кг')
        elif message.text == 'Дополнительно':
            markup = types.ReplyKeyboardMarkup(resize_keyboard=True)
            bot.send_message(message.chat.id, 'Осложенения: инфекционные, экстравазация раствора, выпот в плевральную полость/перикард, холестаз.\n'
                                              'Профилактика тромбозов: 0.5-1ЕД/мл или 25-30ЕД/кг.\n'
